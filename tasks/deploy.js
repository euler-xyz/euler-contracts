...
task("chainlinkOracle:deploy")
    .addOptionalParam("decimals", "decimals", "18")
    .setAction(async (args) => {
        const priceOracleFactory = await ethers.getContractFactory('MockAggregatorProxy');
        
        let tx = await priceOracleFactory.deploy(parseInt(args.decimals));
        console.log(`Transaction: ${tx.deployTransaction.hash}`);
...
task("chainlinkOracle:setValidAnswer")
    .addPositionalParam("impl", "Price oracle smart contract address")
    .addPositionalParam("price")
    .setAction(async (args) => {
        const oracle = await ethers.getContractAt('MockAggregatorProxy', args.impl);
        
        let tx = await oracle.mockSetValidAnswer(args.price);
...
task("deploy:update-network", "Update the current state of Euler smart contracts and markets")
    ...
    .setAction(async () => {
        ...
        // Configuration
        const config = require(`${__dirname}../../test/lib/token-setups/${args.fileName}`);
        ...
        if (config.contracts && config.contracts.length > 0) {
            ...
            if (testnets.includes(networkName)) {
                ...
                throw Error(`${networkName} not supported testnet!\n`);
            }
                ...
                if (token.config) {
                    ...
                    if (currentState.tokens[token.symbol] !== undefined) {
                        ...
                        if (token.config.pricingType === PRICINGTYPE__CHAINLINK && token.config.price && currentState.uniswapPools[token.symbol]) {
                            ...
                            let estimateGasResult = await contracts.governance.estimateGas.setChainlinkPriceFeed(contracts.tokens[token.symbol].address, contracts.chainlinkOracles[token.symbol].address);
                            await (await contracts.governance.connect(deployer).setChainlinkPriceFeed(contracts.tokens[token.symbol].address, contracts.chainlinkOracles[token.symbol].address, { gasLimit: Math.floor(estimateGasResult * 1.01 + 1000) })).wait();

                            // Setup pricing configuration
                            estimateGasResult = await contracts.governance.estimateGas.setPricingConfig(contracts.tokens[token.symbol].address, PRICINGTYPE__CHAINLINK, defaultUniswapFee);
                            await (await contracts.governance.connect(deployer).setPricingConfig(contracts.tokens[token.symbol].address, PRICINGTYPE__CHAINLINK, defaultUniswapFee, { gasLimit: Math.floor(estimateGasResult * 1.01 + 1000) })).wait();
        ...
        console.log("\n Verifying smart contracts via Etherscan.....\n");
...
task("deploy:new-network", "Full deploy of Euler smart contracts and specified test markets")
    ...
    .setAction(async (args) => {
        try {
            ...
            if (config.testing && testnets.includes(networkName)) {
                ...
                if (config.testing.useRealUniswap) {
                    console.log("Deploying Uniswap Contracts....");
                    {
                        ...
                        console.log(`Deployed uniswapV3Factory at: ${contracts.uniswapV3Factory.address}`);
                    }
                    {
                        ...
                        console.log(`Deployed Swap Router at: ${swapRouterAddress}`);
                    }
                    {
                ...
                console.log(`uniswapV3 Byte Code Hash ${uniswapV3PoolByteCodeHash}`);

                contracts.invariantChecker = await (await factories.InvariantChecker.deploy()).deployed();
                verification.push({
                    address: contracts.invariantChecker.address, args: []
                });
                console.log(`Deployed Invariant Checker at: ${contracts.invariantChecker.address}`);

                contracts.flashLoanNativeTest = await (await factories.FlashLoanNativeTest.deploy()).deployed();
                verification.push({
                    address: contracts.flashLoanNativeTest.address, args: []
                });
                console.log(`Deployed Flash Loan Native at: ${contracts.flashLoanNativeTest.address}`);

                contracts.flashLoanAdaptorTest = await (await factories.FlashLoanAdaptorTest.deploy()).deployed();
                verification.push({
                    address: contracts.flashLoanAdaptorTest.address, args: []
                });
                console.log(`Deployed Flash Loan Adaptor at: ${contracts.flashLoanAdaptorTest.address}`);

                contracts.flashLoanAdaptorTest2 = await (await factories.FlashLoanAdaptorTest.deploy()).deployed();
                verification.push({
                    address: contracts.flashLoanAdaptorTest2.address, args: []
                });
                console.log(`Deployed Flash Loan Adaptor 2 at: ${contracts.flashLoanAdaptorTest2.address}`);

                contracts.simpleUniswapPeriphery = await (await factories.SimpleUniswapPeriphery.deploy()).deployed();
                verification.push({
                    address: contracts.simpleUniswapPeriphery.address, args: []
                });
                console.log(`Deployed Simple Uniswap Periphery at: ${contracts.simpleUniswapPeriphery.address}`);
                ...
                for (let pair of config.testing.uniswapPools) {
                    ...
                    console.log(`Pool created for pair: ${pair[0]}/${pair[1]}`);
                }
                ...
                if (config.testing.useRealUniswap) {
                    for (let token of config.testing.activated) {
                        ...
                        console.log(`Pool initialised for ${token}`);
                    }
                }
            } else {
                throw Error(`${networkName} not supported testnet!\n`);
            ...
            console.log("Activating test markets, and setting pricing configurations......\n");
            ...
            if (config.testing && testnets.includes(networkName)) {
                ...
                for (let token of (config.testing.tokens || [])) {
                    ...
                    console.log(`Activated market for ${token.symbol}`);
                }
                ...
                for (let token of (config.testing.tokens || [])) {
                    if (token.config) {
                        ...
                        if (token.config.pricingType === PRICINGTYPE__CHAINLINK && token.config.price) {
                            ...
                            // Setup chainlink prices
                            await (await contracts.chainlinkOracles[token.symbol].connect(deployer).mockSetValidAnswer(ethers.utils.parseEther(`${token.config.price}`))).wait();

                        }
                        console.log(`Configured market for ${token.symbol}`);
                    }
                }
            } else {
                throw Error(`${networkName} not supported testnet!\n`);
            ...
            if (eul !== ethers.constants.AddressZero) {
