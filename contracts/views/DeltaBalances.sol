...
// source: https://etherscan.io/address/0xbf320b8336b131e0270295c15478d91741f9fc11#code
/* 
...
contract DeltaBalances {
    ...
    function withdrawToken(address token, uint256 amount) external isAdmin {
        require(
            token != address(0x0) && Token(token).transfer(msg.sender, amount)
        );
    }
 ...
 */
    function getFunctionSelector(string calldata functionSignature)
        external
        pure
        returns (bytes4)
    {
  ...
  */
    ...
    /* Get multiple token balances deposited on a DEX with a function selector
     ...
     Returns array of deposited token balances in wei units. */
    function depositedBalancesGeneric(
        address exchange,
        bytes4 selector,
        address user,
        address[] calldata tokens,
    ...
    function depositedEtherGeneric(
        address exchange,
        bytes4 selector,
        address user
    ...
    function getNumberTwoArgs(
        ...
        if (isAContract(contractAddr)) {
            (bool success, bytes memory result) = contractAddr.staticcall(
                abi.encodeWithSelector(selector, arg1, arg2)
            );
            // if the contract call succeeded & the result looks good to parse
            if (success && result.length == 32) {
                return abi.decode(result, (uint256)); // return the result as uint
