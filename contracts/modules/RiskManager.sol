...
contract RiskManager is IRiskManager, BaseLogic {
    ...
    function getNewMarketParameters(address underlying) external override returns (NewMarketParameters memory p) {
        ...
        if (underlying == referenceAsset) {
            ...
            (address pool, uint24 fee) = UniswapV3TWAP.findBestUniswapPool(uniswapFactory, underlying, referenceAsset);
            require(pool != address(0), "e/no-uniswap-pool-avail");
            require(UniswapV3TWAP.computeUniswapPoolAddress(uniswapFactory, uniswapPoolInitCodeHash, underlying, referenceAsset, fee) == pool, "e/bad-uniswap-pool-addr");
    ...
    function callUniswapObserve(address underlying, uint pricingParameters, uint twapWindow, uint underlyingDecimalsScaler) private view returns (uint, uint) {
        address pool = UniswapV3TWAP.computeUniswapPoolAddress(uniswapFactory, uniswapPoolInitCodeHash, underlying, referenceAsset, uint24(pricingParameters));
        (uint sqrtPriceX96, uint finalAgo) = UniswapV3TWAP.uniswapObserve(pool, twapWindow);

        return (decodeSqrtPriceX96(underlying, underlyingDecimalsScaler, sqrtPriceX96), finalAgo);
    }
