...
async function main3() {
    ...
    do {
        let temp = amountIn + 1
        if (newDiff > percentageDifference(temp, 300)) {
            amountIn++
            newDiff = percentageDifference(amountIn, 300)
            i++
        } else {
            amountIn--
            newDiff = percentageDifference(Math.abs(amountIn), 300)
            i++
        }
...
async function newBot() {
    ...
    const testToken = "0x19bBa4A58dD289635c48a407542D5B815e4f7094";
    let erc20Token = await token('REP')
    ...
    if (percentageDifference(currPrice, mainNetPrice) > 0.2) {
        ...
        while (newDiff > 0.2); 
        console.log('attempts ', i)
        // todo - precision toFixed, math.abs when reducing amount in
        // todo - to test increase and decrease, reduce required difference to 0.2
        // todo - perform swap here after amountIn and other swap params found
    } else {
...
async function completedBot() {
    ...
    let routerPeriphery = new ethers.Contract(staticSwapRouterPeriphery, staticRouterABI.abi, ctx.wallet);
    // NOTE: to run new bot, static swap router contract needs to have enough balance of the tokens
    const testToken = "0xB7fe2334CD47383C17bfb97B09823F11cc1A91B8";
    let erc20Token = await token('DAI')
    const ropstenWETH = "0xc778417E063141139Fce010982780140Aa0cD5Ab"; //weth
...
//console.log(percentageDifference(121.419, 121.419))