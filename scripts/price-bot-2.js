        ...
        for (let listedToken of tokenPrices) {
                ...
                )
            {
                do {
                    ...
                    if (tempdiff > 1 && tempdiff <= 10) {
                        multiplier = 0.125 / 2 / 2 / 2
                    } 

                    if (tempdiff <= 2 &&
                        parseFloat(Math.abs(parseFloat(mainNetPrice) - parseFloat(currPrice))) > 10) {
                    ...
                    if (tempdiff > 0.2 && tempdiff <= 1 &&
                        parseFloat(mainNetPrice) <= 50 &&
                        parseFloat(currPrice) <= 50) {
                        multiplier = 0.03125
                    }

                    if (tempdiff > 0.3 && tempdiff <= 1 &&
                        parseFloat(mainNetPrice) > 5000 &&
                        parseFloat(currPrice) > 5000) {
                        multiplier = 0.03125
                    }

                    // when the pool balances are small, and there is a wide gap between 
                    // ropsten and mainnet price
                    // a small multiplier does not make
                    // much improvement in the percentage difference, e.g., CRV pool
                    if (testTokenBalance < 50 && wethBalance < 10 & tempdiff > 20) {
                        multiplier = 0.75
                    }

                    // for tokens with high prices 3500 - 10,000
                    // and price difference > 150
                    if (tempdiff > 1 && tempdiff <= 10 && 
                        parseFloat(Math.abs(parseFloat(mainNetPrice) - parseFloat(currPrice))) > 150
                    ) {
                        multiplier = 0.125 / 2 / 2
                    }

                    if (tempdiff > 2 && tempdiff <= 10 && 
                        parseFloat(Math.abs(parseFloat(mainNetPrice) - parseFloat(currPrice))) > 10
                    ) {
                        multiplier = 0.125 / 2 / 2 / 2
                    }

                    if (tempdiff > 1 && tempdiff <= 2 &&
                        parseFloat(Math.abs(parseFloat(mainNetPrice) - parseFloat(currPrice))) > 10) {
                        multiplier = 0.03125 / 2 / 2 / 2 / 2 / 2
                    }

                    if (tempdiff > 2 && tempdiff <= 10 && 
                        parseFloat(mainNetPrice) > 4000 &&
                        parseFloat(currPrice) > 4000
                    ) {
                        multiplier = 0.125 / 2 / 2
                    } else if (tempdiff > 0.8 && tempdiff <= 2 && 
                        parseFloat(mainNetPrice) > 4000 &&
                        parseFloat(currPrice) > 4000) 
                    {
                        multiplier = 0.125 / 2 / 2 / 2 / 2
                    }

                    if (tempdiff > 1 && tempdiff <= 10 && 
                        parseFloat(mainNetPrice) < 30 &&
                        parseFloat(currPrice) < 30
                    ) {
                        multiplier = 0.125 / 2 / 2
                    }

                    if (tempdiff > 10 &&
                        parseFloat(mainNetPrice) < 300 &&
                    ...
                    if (reduce == true) {
                        ...
                        if (parseFloat(mainNetPrice) <  parseFloat(currPrice)) {
                            if (tempdiff > 0.2 &&
                                parseFloat(Math.abs(parseFloat(mainNetPrice) - parseFloat(currPrice))) > 20) {
                    ...
                    // todo log token symbol
                    console.log('[attempts] #', i, listedToken.token)
